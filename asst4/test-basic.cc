...
TEST_F(BasicTest, TEST_NAME) { \
    ...
    bool ret = false; \
    bool ok = true; \
    uint32_t in_size = 0, out_size = 0, temp; \
    \
    ASSERT_FALSE(test.Encode(in_buf, &in_len)) << \
        "Encode<" << tname << "> returned true on nullptr buffer"; \
    \
    for (in_len = 1; in_len <= MAX_LEN; in_len *= 2) { \
        in_buf = new uint8_t[in_len];    \
        in_size = in_len; \
        if ((ret = test.Encode(in_buf, &in_size))) { \
            break; \
        } \
        else { \
            delete [] in_buf; \
    ...
    \
    EXPECT_LE(in_size, in_len) << "Encode<" << tname << "> set *out_len to " \
    ...
    EXPECT_EQ(temp, in_size) << "Encode<" << tname << "> set *out_len to " \
        << temp << " on buffer size of " << in_len * 2 << " bytes."; \
    \
    ASSERT_FALSE(test.Decode(out_buf, &out_len, &ok)) \
        << "Decode<" << tname << "> returned true on nullptr buffer"; \
    for (out_len = 1; out_len < in_len; out_len *= 2) { \
        out_buf = new uint8_t[out_len]; \
        memcpy(out_buf, in_buf, out_len); \
        out_size = out_len; \
        ret = test.Decode(out_buf, &out_size, &ok); \
        EXPECT_FALSE(ret) << "Encode<" << tname << "> required at least " \
            << in_len << " bytes, but Decode<" << tname << "> returns " \
            << "true on " << out_len << " bytes!"; \
        delete [] out_buf; \
    } \
    \
    out_buf = new uint8_t[out_len]; \
    ...
    ASSERT_TRUE(ret) << "Decode<" << tname << "> returns false on " \
        << out_len << " bytes, but Encode<" << tname << "> succeeded on " \
        << in_len << " bytes!"; \
    EXPECT_LE(out_size, out_len) << "Decode<" << tname << "> set *in_len to " \
    ...
    EXPECT_EQ(ok, true); \
    EXPECT_EQ(in_size, out_size) << "Mismatch on amount encoded vs. amount " \
        << "decoded"; \
    ...
    ASSERT_EQ(temp, in_len * 2); \
    memcpy(out_buf, in_buf, temp); \
    EXPECT_TRUE(test.Decode(out_buf, &temp, &ok)) << "Decode<" << tname << \
        "> failed on buffer size of " << out_len * 2 << " bytes."; \
    EXPECT_EQ(temp, out_size) << "Decode<" << tname << "> set *in_len to " \
